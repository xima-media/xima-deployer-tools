...
class TemplateService
{
    ...
    private function renderIssueData(Entry $entry): string {
        ...
        $issueType = !empty($entry->getIssueData()) ? "<img src='" . $entry->getIssueData()['type']['icon'] . "'/>" : "";
        $issue = $entry->getIssue() ? "<a class='pill' href='" . $entryUtility->getIssueLink($entry) . "' data-tooltip='Jira issue " . $entry->getIssue() . "' target='_blank'>$issueType " . $entry->getIssue() . "</a>" : "";
        $issueStatus = !empty($entry->getIssueData()) ? "<kbd class='" . $entry->getIssueData()['status']['color'] . "' data-tooltip='Jira issue status' style='font-size: 50%;'>" . $entry->getIssueData()['status']['name'] . "</kbd>" : "";
        $issueAssignee = !empty($entry->getIssueData()) ? "<span class='pill' data-tooltip='Jira issue assignee' style='font-size: 50%;'>" . $entry->getIssueData()['assignee']['name'] . "</span>" : "";

        return "$issueStatus $issueAssignee $issue";
    }
