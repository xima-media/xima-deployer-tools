...
task('deploy:database:update', function () {
  runExtended('cd {{drupal_site_path}} && {{drush_binary}} updb -y');
});
...
task('deploy:database:sync', function () {
  ...
  runExtended("cd {{drupal_site_path}} && {{drush_binary}} sql:sync @$source @self --create-db -y");
})->select('type=feature-branch-deployment');

desc('Backup database');
task('deploy:database:backup', function () {
  if (has('previous_release') && get('previous_release')) {
    runExtended("cd {{previous_release}} && {{drush_binary}} sql-dump --structure-tables-list=cache,cache_*,queue,watchdog --gzip --result-file=auto");
