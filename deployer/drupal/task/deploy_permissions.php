...
task('deploy:permissions:drupal', function () {
  writeln('<info>↪ Changing permissions of all directories inside "{{drupal_path}}" which are owned by {{remote_user}} to "rwxr-x---"</info>');
  run ('cd {{drupal_path}} && find . -type d -user {{remote_user}} -exec chmod u=rwx,g=rx,o= \'{}\' \;');

  writeln('<info>↪ Changing permissions of all files inside "{{drupal_path}}" which are owned by {{remote_user}} to "rw-r-----"</info>');
...
task('deploy:permissions:drupal_files', function () {
  writeln('<info>↪ Changing permissions of "files" directories in "{{drupal_path}}/sites" which are owned by {{remote_user}} to "rwxrwx---"</info>');
  run ('cd {{drupal_path}}/sites && find . -type d -name files -user {{remote_user}} -exec chmod ug=rwx,o= \'{}\' \;');

  writeln('<info>↪ Changing permissions inside "files" directories in "{{drupal_path}}/sites" of all directories which are owned by {{remote_user}} to "rwxrwx---" and files to "rw-rw----"</info>');
  ...
  writeln('<info>↪ Changing permissions inside "files" directories in "{{drupal_path}}/sites" of all .htaccess-files which are owned by {{remote_user}} to "rw-r-----"</info>');
